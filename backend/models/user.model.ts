...
import type { Id, UserProfile } from "../types.js";
...
async function insertUser(user: UserInsert): Promise<Id> {
    ...
    await Promise.all((user.roles ?? []).map(role => addRoleToUser(userId, role)));
    return userId;
}
...
async function findUser(id: Id): Promise<User | null> {
    const query = `
        SELECT u.*, GROUP_CONCAT(DISTINCT ur.role_id) as roles
        FROM ${USER_TABLE} u
        LEFT JOIN ${USER_ROLE_TABLE} ur ON u.id = ur.user_id
...
async function findUserProfile(id: Id): Promise<UserProfile | null> {
