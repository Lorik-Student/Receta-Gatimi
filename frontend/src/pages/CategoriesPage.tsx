...
export async function categoriesLoader() {
  ...
  const categories = Array.isArray(categoriesResult)
    ? categoriesResult
    : ((categoriesResult as any).data || Object.values(categoriesResult).filter(v => typeof v === "object" && v !== null && "id" in v));
  const recipes = Array.isArray(recipesResult)
    ? recipesResult
    : ((recipesResult as any).data || Object.values(recipesResult).filter(v => typeof v === "object" && v !== null && "id" in v));
...
export function CategoriesPage() {
  ...
  const recipeCards: RecipeCardData[] = filteredRecipes.map((r: any) => ({
    ...
    badge: categories.find((c: any) => c.id === r.category_id)?.emertimi || "Të përgjithshme",
  ...
  return (
    <div className="bg-background text-on-background font-body-md min-h-screen flex flex-col">
      <Header brand="Receta Gatimi" activePath="/categories" />
      <main className="flex-1 max-w-container-max-width mx-auto px-margin-desktop py-12 w-full">
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="font-body-lg text-on-surface-variant">Eksploro recetat sipas kategorive.</p>
          </div>
        </div>
